            ...
            <Card className="p-4 bg-white/70 backdrop-blur-sm border-0">
              <div className="flex flex-wrap items-center gap-4">
                ...
                <div className="flex gap-2">
                  {["all", "electronics", "audio", "footwear", "kitchen", "home"].map((category) => (
                    <Button
                      key={category}
                      ...
                      className={`capitalize ${
                        filters.category === category ? "bg-gradient-to-r from-indigo-500 to-purple-600" : ""
                      }`}
                    >
                      {category}
                    </Button>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button
                        key={star}
                        onClick={() => handleFilterChange("minRating", star === filters.minRating ? 0 : star)}
                        className={`text-lg ${star <= filters.minRating ? "text-yellow-400" : "text-gray-300"}`}
                      >
                        ⭐
                      </button>
                    ))}
        ...
        {!isLoading && results.length > 0 && (
          <div className="max-w-6xl mx-auto px-4 pb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {results.toSorted((a: Product, b: Product) => parseFloat(a.price.replace(/[^\d.-]/g, "")) - parseFloat(b.price.replace(/[^\d.-]/g, ""))).map((product, index) => (
                <Card
                  key={product.id}
